...
int handle_magnet_handshake(const string ip, const uint16_t port, const string hash, uint8_t &metadata_id) {
    ...
    metadata_id = extension_object["m"]["ut_metadata"].get<uint8_t>();
    cout << "handle_magnet_handshake = " << metadata_id << endl;
...
json handle_magnet_info(const int sockfd, unsigned char metadata_id, unsigned int piece) {
    ...
    //   begin++;
    //   while (s[begin] != 'e') {
    //     json key = decode_bencoded_value(s, begin);
    //     json value = decode_bencoded_value(s, begin);
    //     object[key] = value;
    //   }
    //   begin++;
    // }
